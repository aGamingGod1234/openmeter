...
import { accountLabel, createNamedAccount, normalizeAccountId, renameAccount } from "../src/accounts-ui";
...
describe("multi-account settings", () => {
  it("creates and renames a stable sibling card without changing its layout key", () => {
    const account = createNamedAccount("claude", "Work Account", "Work", "D:\\AI\\claude-work");
    expect(account.account_id).toBe("work-account");
    expect(account.card_id).toBe("claude--work-account");
    expect(account.source).toEqual({ kind: "directory", path: "D:\\AI\\claude-work" });
  ...
  it("rejects reserved IDs and keeps sibling layouts independent", () => {
    ...
    expect(() => createNamedAccount("codex", "default", "Work", "D:\\codex")).toThrow();
    ...
    const layout = migrateLayout(null, [
      base,
      { ...base, id: "claude--work", account_id: "work", card_id: "claude--work", stale: true },
    ]);
