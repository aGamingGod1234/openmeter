...
# (no admin), and launches OpenMeter. Works on Windows PowerShell 5.1 and pwsh.
...
Write-Host ''
Write-Host '  OpenMeter - AI usage limits in your system tray' -ForegroundColor Cyan
Write-Host ''

Write-Host '> Finding the latest release...'
$release = Invoke-RestMethod 'https://api.github.com/repos/aGamingGod1234/openmeter/releases/latest' `
    -Headers @{ 'User-Agent' = 'openmeter-installer' } -UseBasicParsing
$asset = $release.assets | Where-Object { $_.name -match '^OpenMeter_.+_x64-setup\.exe$' } | Select-Object -First 1
if (-not $asset) { throw 'No installer found in the latest release - please report this at https://github.com/aGamingGod1234/openmeter/issues' }
...
# Integrity check — MANDATORY, fails closed. OpenMeter's release process writes
...
if (-not $manifest) {
    ...
    throw 'Release is missing its latest.json manifest - not installing. Please report this at https://github.com/aGamingGod1234/openmeter/issues'
}
$expected = (Invoke-RestMethod $manifest.browser_download_url -UseBasicParsing).sha256
if (-not $expected) {
    Remove-Item $dest -Force -ErrorAction SilentlyContinue
    throw 'Release manifest has no sha256 field - not installing. Please report this at https://github.com/aGamingGod1234/openmeter/issues'
}
$actual = (Get-FileHash $dest -Algorithm SHA256).Hash.ToLower()
if ($actual -ne $expected.ToLower()) {
    Remove-Item $dest -Force -ErrorAction SilentlyContinue
    throw 'Integrity check FAILED - download does not match the release manifest SHA-256. Not installing. Please report this at https://github.com/aGamingGod1234/openmeter/issues'
}
...
Start-Process $exe
Write-Host ''
Write-Host ("  OpenMeter {0} installed - look for the icon in your system tray (next to the clock)." -f $release.tag_name) -ForegroundColor Green
